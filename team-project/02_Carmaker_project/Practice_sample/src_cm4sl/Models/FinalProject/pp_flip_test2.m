...
fprintf('reverse-in (Dr floor 1.0): okF=%d chosen Dr=%.1f  staging sf=(%.2f,%.2f)\n',okF,Dr,sfx,sfy);
px=zeros(MAXP,1);py=px;pth=px;pdir=px; okp=false; np=0;
if okF, [px,py,pth,pdir,np,okp]=pp_hybrid_astar(ex,ey,eth,sfx,sfy,gth,occ_plan); dirv=-1; if okp,[px,py,pth,pdir,np]=pp_append_straight(px,py,pth,pdir,np,gx,gy,gth,cfg.ds,MAXP,dirv);end; end
...
fprintf('BACK-IN -pi/2: okp=%d np=%d switches=%d final err=%.2fm onOCC=%d last6=%s\n',okp,np,sw,hypot(px(max(np,1))-gx,py(max(np,1))-gy),offc,mat2str(pdir(max(1,np-5):max(np,1))'));
% min distance from any path point's vehicle CENTER-LINE to T06 center, plus footprint clearance proxy
if okp, dT06=min(hypot(px(1:np)-7,py(1:np)+21.8)); fprintf('min path-point dist to T06 center=%.2fm (T06 half-diag ~2.4m + ego half ~1)\n',dT06); end
disp('BACKIN_FLOOR1_DONE');
