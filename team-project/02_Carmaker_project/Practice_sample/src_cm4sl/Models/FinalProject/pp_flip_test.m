% build real-surface chart-equivalent map
...
for k=1:size(OB,1)
  x=OB(k,1);y=OB(k,2);th=OB(k,3); co=cos(th);si=sin(th); lx=[ah;ah;-bh;-bh]; ly=[ohw;-ohw;-ohw;ohw]; c=[x+co*lx-si*ly, y+si*lx+co*ly]; e=1e-9; pos=true(n,n); neg=true(n,n);
...
ego_x=-0.294; ego_y=-34.512; ego_yaw=deg2rad(-52.9); fin=[9 -11]; gth=-pi/2;   % 180-deg flipped (nose south)
...
fprintf('goal(9,-11,-pi/2) footprint_free=%d\n',gf);
% adaptive reverse-first (current F6 logic)
Dst=cfg.stage_dist; sbx=gx-Dst*cos(gth); sby=gy-Dst*sin(gth); okF=false; sfx=gx; sfy=gy; Dr=Dst;
while Dr>=1.5, tfx=gx+Dr*cos(gth); tfy=gy+Dr*sin(gth); if ~pp_collision(occ_plan,tfx,tfy,gth,cfg.foot_ahead,cfg.foot_behind,cfg.veh_halfW,cfg.bounds,cfg.n), sfx=tfx;sfy=tfy;okF=true;break; end; Dr=Dr-0.5; end
okB=~pp_collision(occ_plan,sbx,sby,gth,cfg.foot_ahead,cfg.foot_behind,cfg.veh_halfW,cfg.bounds,cfg.n);
fprintf('reverse-in staging okF=%d (chosen Dr=%.1f) | forward-in staging okB=%d\n',okF,Dr,okB);
...
sw=sum(abs(diff(sign(pdir(pdir~=0))))>0); offc=0;
for i=1:np, cxx=max(min(floor((px(i)-b(1))/rx)+1,n),1); cyy=max(min(floor((py(i)-b(3))/ry)+1,n),1); offc=offc+(occ(cyy,cxx)>0); end
fprintf('RESULT: okp=%d np=%d switches=%d final_dir=%d (-1=reverse/후진, +1=forward/전진)\n',okp,np,sw,dirv);
if np>0, fprintf('final err=%.2fm last6dir=%s onOCC=%d/%d\n',hypot(px(np)-gx,py(np)-gy),mat2str(pdir(max(1,np-5):np)'),offc,np); end
disp('FLIP_TEST_DONE');
