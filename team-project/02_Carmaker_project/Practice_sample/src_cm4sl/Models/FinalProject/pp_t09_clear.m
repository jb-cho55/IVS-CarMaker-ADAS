...
% T09 footprint corners (14,-21.9,+pi/2) and perimeter samples
tx=14; ty=-21.9; tth=pi/2; cco=cos(tth);ss=sin(tth); Tc=[tx+cco*[oL;oL;0;0]-ss*[oW/2;-oW/2;-oW/2;oW/2], ty+ss*[oL;oL;0;0]+cco*[oW/2;-oW/2;-oW/2;oW/2]];
Tperim=[]; for i=1:4, a=Tc(i,:); bb=Tc(mod(i,4)+1,:); tt=linspace(0,1,25)'; Tperim=[Tperim; a+(bb-a).*tt]; end
for mg=[0.40 0.70]
  ...
  if okF, [px,py,pth,pdir,np,okp]=pp_hybrid_astar(ex,ey,eth,sfx,sfy,gth,occ_plan); if okp,[px,py,pth,pdir,np]=pp_append_straight(px,py,pth,pdir,np,gx,gy,gth,cfg.ds,MAXP,-1);end; end
  % min ego-footprint -> T09 distance over path
  fa=cfg.foot_ahead; fbk=cfg.foot_behind; hw=cfg.veh_halfW; dmin=inf; iat=0;
  for i=1:np
    co=cos(pth(i));si=sin(pth(i)); E=[px(i)+co*[fa;fa;-fbk;-fbk]-si*[hw;-hw;-hw;hw], py(i)+si*[fa;fa;-fbk;-fbk]+co*[hw;-hw;-hw;hw]];
    Ep=[]; for j=1:4, a=E(j,:); bb=E(mod(j,4)+1,:); tt=linspace(0,1,18)'; Ep=[Ep; a+(bb-a).*tt]; end
    D=hypot(Ep(:,1)-Tperim(:,1)', Ep(:,2)-Tperim(:,2)'); dd=min(D(:));
    if any(inpolygon(Ep(:,1),Ep(:,2),Tc(:,1),Tc(:,2))) || any(inpolygon(Tperim(:,1),Tperim(:,2),E(:,1),E(:,2))), dd=0; end
    if dd<dmin, dmin=dd; iat=i; end
  end
  fprintf('margin=%.2f: okp=%d np=%d | MIN ego-to-T09 clearance=%.2fm at idx %d (dir=%d, y=%.1f)\n', mg, okp, np, dmin, iat, pdir(max(iat,1)), py(max(iat,1)));
end
disp('T09CLEAR_DONE');
