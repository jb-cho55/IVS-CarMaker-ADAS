n=200; b=[-5 48 -45.5 6]; rx=(b(2)-b(1))/n; ry=(b(4)-b(3))/n; xc=b(1)+((1:n)-0.5)*rx; yc=b(3)+((1:n)-0.5)*ry; [XC,YC]=meshgrid(xc,yc); em=0.20; mg=0.80;
base_free=(XC>=-2.5+em&XC<=2.5-em&YC>=-36.6&YC<=6)|(XC>=0&XC<=47.6-em&YC>=-39.1+em&YC<=-34.1)|(XC>=5.5+em&XC<=47.6-em&YC>=-34.1&YC<=-4.1-em)|(XC>=4&XC<=5.5&YC>=-34.1&YC<=-30.1);
...
oL=4.47; oW=1.97; cfg=pp_cfg(); d_r=cfg.d_r; MAXP=cfg.max_path;
M=ones(n,n); M(base_free)=0; ah=oL+mg; bh=mg; ohw=oW/2+mg;
...
ego_x=-0.294; ego_y=-34.512; ego_yaw=deg2rad(-52.9); fin=[9 -6.8]; gth=-pi/2; Dv=1.5;
...
sfx=gx+Dv*cos(gth); sfy=gy+Dv*sin(gth); px=zeros(MAXP,1);py=px;pth=px;pdir=px;
[px,py,pth,pdir,np,okp]=pp_hybrid_astar(ex,ey,eth,sfx,sfy,gth,occ_plan); if okp,[px,py,pth,pdir,np]=pp_append_straight(px,py,pth,pdir,np,gx,gy,gth,cfg.ds,MAXP,-1);end
sw=sum(abs(diff(sign(pdir(pdir~=0))))>0); L=sum(hypot(diff(px(1:np)),diff(py(1:np))));
offc=0; for i=1:np,cxx=max(min(floor((px(i)-b(1))/rx)+1,n),1);cyy=max(min(floor((py(i)-b(3))/ry)+1,n),1);offc=offc+(occ(cyy,cxx)>0);end
rb_y=py(np)+cfg.foot_behind*(-sin(gth));
fprintf('BEST mg=0.8 Dr=1.5: okp=%d np=%d len=%.1f switches=%d onOCC=%d finaldir=%d\n',okp,np,L,sw,offc,pdir(np));
fprintf('ego rear-bumper Y=%.2f (T01=-6.80, diff=%.2f) | err=%.2f\n', rb_y, rb_y+6.8, hypot(px(np)-gx,py(np)-gy));
d=pdir(1:np); s0=1; fprintf('--- segments ---\n'); for i=2:np, if sign(d(i))~=sign(d(i-1))||i==np, e2=i-1; if i==np,e2=i;end; seglen=sum(hypot(diff(px(s0:e2)),diff(py(s0:e2)))); fprintf(' dir=%+d (%.1f,%.1f)->(%.1f,%.1f) len=%.1fm\n',d(s0),px(s0),py(s0),px(e2),py(e2),seglen); s0=i; end; end
assignin('base','BST',struct('px',px(1:np),'py',py(1:np),'pdir',pdir(1:np),'np',np,'gx',gx,'gy',gy,'gth',gth,'ex',ex,'ey',ey,'eth',eth,'occ',occ,'OB',OB,'xc',xc,'yc',yc,'sfx',sfx,'sfy',sfy));
% figure
fig=figure('Color','w','Position',[60 40 900 850]); hold on; img=cat(3,1-0.75*(occ>0),1-0.7*(occ>0),1-0.55*(occ>0)); image(xc,yc,img); set(gca,'YDir','normal');
for k=1:size(OB,1), x=OB(k,1);y=OB(k,2);th=OB(k,3); co=cos(th);si=sin(th); c=[x+co*[oL;oL;0;0]-si*[oW/2;-oW/2;-oW/2;oW/2], y+si*[oL;oL;0;0]+co*[oW/2;-oW/2;-oW/2;oW/2]]; cl=[0.55 0.6 0.66]; if any(k==[7 10]),cl=[0.95 0.55 0.2];end; patch(c([1:4 1],1),c([1:4 1],2),cl,'EdgeColor',[0.2 0.22 0.26]); text(x+co*oL/2,y+si*oL/2,sprintf('T%02d',k-1),'FontSize',6.5,'Color','w','HorizontalAlignment','center'); end
fwd=pdir(1:np)>=0; plot(px(1:np),py(1:np),'-','Color',[.5 .5 .5]); scatter(px(find(fwd)),py(find(fwd)),15,'filled','MarkerFaceColor',[0 0.5 0.95]); scatter(px(find(~fwd)),py(find(~fwd)),22,'filled','MarkerFaceColor',[0.95 0.2 0]);
de=@(X,Y,T,c)patch(X+cos(T)*[3.73;3.73;-0.95;-0.95]-sin(T)*[0.94;-0.94;-0.94;0.94], Y+sin(T)*[3.73;3.73;-0.95;-0.95]+cos(T)*[0.94;-0.94;-0.94;0.94], c,'FaceAlpha',0.4,'EdgeColor',c*0.6,'LineWidth',1.6);
de(ex,ey,eth,[1 0.7 0.2]); de(px(np),py(np),gth,[0.2 0.8 0.3]); yline(-6.8,'m--','LineWidth',1.0);
text(9,-9,'목표 (9,-6.8,-\pi/2)','Color',[0 0.5 0.1],'FontWeight','bold','FontSize',10,'HorizontalAlignment','center'); text(13.6,-6.8,'T01','FontSize',8);
axis equal; xlim([-6 48]); ylim([-42 4]); grid on; xlabel('X[m]');ylabel('Y[m]'); title('\bf개선: 정렬거리1.5+여유0.8 → 기어2회, 모든차서 \geq0.8m','FontSize',12);
png='D:\HL_IVS_School\Code\motionplanning\26HL_IVS_ADAS\02_Carmaker_project\Practice_sample\src_cm4sl\Models\FinalProject\parking_safe_v2.png'; exportgraphics(fig,png,'Resolution',150); fprintf('saved %s\n',png); disp('VBEST_DONE');
