...
n=200; b=[-5 48 -45.5 6]; rx=(b(2)-b(1))/n; ry=(b(4)-b(3))/n; chk=@(x,y) occ(max(min(floor((y-b(3))/ry)+1,n),1), max(min(floor((x-b(1))/rx)+1,n),1));
fprintf('free fraction=%.1f%%\n',100*mean(occ(:)==0));
fprintf('T06 infl 0.8: (7.0,-19.4)=%d exp1(0.6m)  (7.0,-19.0)=%d exp0(1.0m)\n', chk(7.0,-19.4), chk(7.0,-19.0));
fprintf('goal(9,-6.8)=%d exp0 | entry(-0.29,-34.5)=%d exp0\n', chk(9,-6.8), chk(-0.29,-34.5));
H=-pi/2; aL=3.73; aB=0.95; bw=0.94; co=cos(H);si=sin(H); xs=9+co*[aL;aL;-aB;-aB]-si*[bw;-bw;-bw;bw]; ys=-6.8+si*[aL;aL;-aB;-aB]+co*[bw;-bw;-bw;bw];
[XX,YY]=meshgrid(b(1)+((1:n)-0.5)*rx,b(3)+((1:n)-0.5)*ry); ip=inpolygon(XX,YY,xs,ys); fprintf('goal footprint occ cells=%d of %d exp0\n', sum(occ(ip)>0), sum(ip(:)));
disp('MAPCHK2_DONE');
...
cfg.margin      = 0.80;                   % obstacle footprint safety inflation [m] (raised 0.40->0.70: more clearance from parked cars e.g. T09 during reverse, for tracking error)
