% ===== VERIFY edited-chart map (real surface) =====
map0 = pp_generate_map([],[],[]); occ = pp_add_obstacle(map0,[],[]);
n=200; b=[-5 48 -45.5 6]; rx=(b(2)-b(1))/n; ry=(b(4)-b(3))/n;
g2i=@(x,y) deal(min(max(floor((x-b(1))/rx)+1,1),n), min(max(floor((y-b(3))/ry)+1,1),n));
chk=@(x,y) occ(max(min(floor((y-b(3))/ry)+1,n),1), max(min(floor((x-b(1))/rx)+1,n),1));
fprintf('free fraction = %.1f%%\n',100*mean(occ(:)==0));
fprintf('NEW goal (21.45,-21.8): occ=%d (want 0 free)\n', chk(21.45,-21.8));
fprintf('entry (-0.29,-34.5)   : occ=%d (want 0 free)\n', chk(-0.29,-34.5));
fprintf('strip (12,-36.6)      : occ=%d (want 0 free)\n', chk(12,-36.6));
fprintf('bay aisle (21.45,-27) : occ=%d (want 0 free)\n', chk(21.45,-27));
fprintf('--- void/shoulder (want 1 occupied) ---\n');
fprintf('shoulder (4,-15)  : occ=%d\n', chk(4,-15));
fprintf('shoulder (3.5,-25): occ=%d\n', chk(3.5,-25));
fprintf('crash (3.42,-40.24): occ=%d\n', chk(3.42,-40.24));
fprintf('far N (21.45,-2)  : occ=%d\n', chk(21.45,-2));
% goal footprint free-cells (between T07/T08)
H=pi/2; aL=3.73; aB=0.95; bw=0.94; gx=21.45; gy=-21.8; co=cos(H);si=sin(H);
xs=gx+co*[aL;aL;-aB;-aB]-si*[bw;-bw;-bw;bw]; ys=gy+si*[aL;aL;-aB;-aB]+co*[bw;-bw;-bw;bw];
[XX,YY]=meshgrid(b(1)+((1:n)-0.5)*rx, b(3)+((1:n)-0.5)*ry); ip=inpolygon(XX,YY,xs,ys);
fprintf('NEW goal footprint occupied cells = %d of %d (want 0)\n', sum(occ(ip)>0), sum(ip(:)));
assignin('base','occ_real',occ); disp('VERIFY2_DONE');
...
if isempty(BASE)
    ...
    % --- carve the REAL drivable surface FREE (IPGRoad geometry, not a rectangle) -
    %   Driving lanes (type 0) + parking bay (type 13), reconstructed from the
    %   TestRun road links/lanes. surf rows = [x0 x1 y0 y1], ALREADY inset by the
    %   edge margin on OUTER paved walls only; internal seams (Vroad<->Hstrip and
    %   Hstrip<->Bay) are left flush so the vehicle can cross between them.
    %   Everything else inside cfg.bounds stays OCCUPIED (no cutting across void).
    %     Vroad : x~0 vertical driving road     world x[-2.5,2.5] y[-36.6,6]
    %     Hstrip: y~-36.6 driving strip (type0)  world x[0,47.6]   y[-39.1,-34.1]
    %     Bay   : type-13 parking bay (north)    world x[5.5,47.6] y[-34.1,-4.1]
    %     BayW  : narrow west sliver of the bay  world x[4,5.5]    y[-34.1,-30.1]
    surf = [ -2.3  2.3  -36.6    6.0 ;
              0.0 47.4  -38.9  -34.1 ;
              5.7 47.4  -34.1   -4.3 ;
              4.0  5.5  -34.1  -30.1 ];
    for r = 1:size(surf,1)
        [sx0, sy0] = pp_world2grid(surf(r,1), surf(r,3), b, n);
        [sx1, sy1] = pp_world2grid(surf(r,2), surf(r,4), b, n);
        for ix = min(sx0,sx1):max(sx0,sx1)
            for iy = min(sy0,sy1):max(sy0,sy1)
                M(iy, ix) = 0;
            end
        end
    end
