n=200; b=[-5 48 -45.5 6]; rx=(b(2)-b(1))/n; ry=(b(4)-b(3))/n; xc=b(1)+((1:n)-0.5)*rx; yc=b(3)+((1:n)-0.5)*ry; [XC,YC]=meshgrid(xc,yc); em=0.20;
Vroad=(XC>=-2.5+em&XC<=2.5-em&YC>=-36.6&YC<=6); Hstrip=(XC>=0&XC<=47.6-em&YC>=-39.1+em&YC<=-34.1); Bay=(XC>=5.5+em&XC<=47.6-em&YC>=-34.1&YC<=-4.1-em); BayW=(XC>=4&XC<=5.5&YC>=-34.1&YC<=-30.1);
...
mg=0.40; oL=4.47; oW=1.97; ah=oL+mg; bh=mg; ohw=oW/2+mg;
for k=1:size(OB,1), x=OB(k,1);y=OB(k,2);th=OB(k,3); co=cos(th);si=sin(th); lx=[ah;ah;-bh;-bh]; ly=[ohw;-ohw;-ohw;ohw]; c=[x+co*lx-si*ly, y+si*lx+co*ly]; e=1e-9; pos=true(n,n);neg=true(n,n); for kk=1:4,k2=mod(kk,4)+1;ex2=c(k2,1)-c(kk,1);ey2=c(k2,2)-c(kk,2);sj=ex2*(YC-c(kk,2))-ey2*(XC-c(kk,1));pos=pos&(sj>=-e);neg=neg&(sj<=e);end; M(pos|neg)=1; end
w=1; M(1:w,:)=1;M(n-w+1:n,:)=1;M(:,1:w)=1;M(:,n-w+1:n)=1; occ=M;
cfg=pp_cfg(); d_r=cfg.d_r; MAXP=cfg.max_path;
ego_x=-0.294; ego_y=-34.512; ego_yaw=deg2rad(-52.9); fin=[9 -11]; gth=-pi/2;
[ex,ey]=pp_axle_from_fr1(ego_x,ego_y,ego_yaw,d_r); eth=ego_yaw; [gx,gy]=pp_axle_from_fr1(fin(1),fin(2),gth,d_r);
occ_plan=pp_clear_goal(occ,fin(1),fin(2),gth,cfg);
% adaptive reverse-first, floor 1.0 (mirrors edited F6)
Dst=cfg.stage_dist; sbx=gx-Dst*cos(gth); sby=gy-Dst*sin(gth); okF=false; sfx=gx; sfy=gy; Dr=Dst;
while Dr>=1.0, tfx=gx+Dr*cos(gth); tfy=gy+Dr*sin(gth); if ~pp_collision(occ_plan,tfx,tfy,gth,cfg.foot_ahead,cfg.foot_behind,cfg.veh_halfW,cfg.bounds,cfg.n), sfx=tfx;sfy=tfy;okF=true;break; end; Dr=Dr-0.5; end
okB=~pp_collision(occ_plan,sbx,sby,gth,cfg.foot_ahead,cfg.foot_behind,cfg.veh_halfW,cfg.bounds,cfg.n);
px=zeros(MAXP,1);py=px;pth=px;pdir=px; okp=false; np=0; dirv=1;
if okF, [px,py,pth,pdir,np,okp]=pp_hybrid_astar(ex,ey,eth,sfx,sfy,gth,occ_plan); dirv=-1; if ~okp&&okB,[px,py,pth,pdir,np,okp]=pp_hybrid_astar(ex,ey,eth,sbx,sby,gth,occ_plan);dirv=1;end
elseif okB, [px,py,pth,pdir,np,okp]=pp_hybrid_astar(ex,ey,eth,sbx,sby,gth,occ_plan); dirv=1; end
if okp, [px,py,pth,pdir,np]=pp_append_straight(px,py,pth,pdir,np,gx,gy,gth,cfg.ds,MAXP,dirv); end
sw=sum(abs(diff(sign(pdir(pdir~=0))))>0); offc=0; for i=1:np,cxx=max(min(floor((px(i)-b(1))/rx)+1,n),1);cyy=max(min(floor((py(i)-b(3))/ry)+1,n),1);offc=offc+(occ(cyy,cxx)>0);end
fprintf('FLIPPED back-in (9,-11,-pi/2): okF=%d Dr=%.1f okp=%d np=%d switches=%d final_dir=%d err=%.2f onOCC=%d\n',okF,Dr,okp,np,sw,dirv,hypot(px(np)-gx,py(np)-gy),offc);
co=cos(gth);si=sin(gth); fprintf('final nose Y=%.2f (bay south is open), rear Y=%.2f\n', py(np)+co*0+si*3.73, py(np)-si*0.95*0+ (py(np)+ -si*0.95) );
% figure
fig=figure('Color','w','Position',[60 40 940 860]); hold on;
img=cat(3,1-0.75*(occ>0),1-0.7*(occ>0),1-0.55*(occ>0)); image(xc,yc,img); set(gca,'YDir','normal');
for k=1:size(OB,1), x=OB(k,1);y=OB(k,2);th=OB(k,3); co=cos(th);si=sin(th); c=[x+co*[oL;oL;0;0]-si*[oW/2;-oW/2;-oW/2;oW/2], y+si*[oL;oL;0;0]+co*[oW/2;-oW/2;-oW/2;oW/2]]; patch(c([1:4 1],1),c([1:4 1],2),[0.55 0.6 0.66],'EdgeColor',[0.2 0.22 0.26]); text(x+co*oL/2,y+si*oL/2,sprintf('T%02d',k-1),'FontSize',6.5,'Color','w','HorizontalAlignment','center'); end
fwd=pdir(1:np)>=0; plot(px(1:np),py(1:np),'-','Color',[.5 .5 .5]); scatter(px(find(fwd)),py(find(fwd)),15,'filled','MarkerFaceColor',[0 0.5 0.95]); scatter(px(find(~fwd)),py(find(~fwd)),24,'filled','MarkerFaceColor',[0.95 0.2 0]);
de=@(X,Y,T,c)patch(X+cos(T)*[3.73;3.73;-0.95;-0.95]-sin(T)*[0.94;-0.94;-0.94;0.94], Y+sin(T)*[3.73;3.73;-0.95;-0.95]+cos(T)*[0.94;-0.94;-0.94;0.94], c,'FaceAlpha',0.4,'EdgeColor',c*0.6,'LineWidth',1.5);
de(ex,ey,eth,[1 0.7 0.2]); de(sfx,sfy,gth,[0.3 0.3 0.9]); de(px(np),py(np),gth,[0.2 0.8 0.3]);
text(9,-9,'목표 (9,-11,-\pi/2): 코앞 남향, 후진주차','Color',[0 0.5 0.1],'FontWeight','bold','FontSize',10,'HorizontalAlignment','center'); text(13.6,-6.8,'T01','FontSize',8);
legend({'','경로','전진','후진'},'Location','southeast'); axis equal; xlim([-6 48]); ylim([-42 4]); grid on; xlabel('X[m]');ylabel('Y[m]');
title({'\bf주차방향 180° 회전: T01 옆 (9,-11) 코앞 남향 후진주차','빨강=후진 진입 구간'},'FontSize',12);
png='D:\HL_IVS_School\Code\motionplanning\26HL_IVS_ADAS\02_Carmaker_project\Practice_sample\src_cm4sl\Models\FinalProject\parking_rear_flipped.png'; exportgraphics(fig,png,'Resolution',150); fprintf('saved %s\n',png);
disp('FLIP_FINAL_DONE');
