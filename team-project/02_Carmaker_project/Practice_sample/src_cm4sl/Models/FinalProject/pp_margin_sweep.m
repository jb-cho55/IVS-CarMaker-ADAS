...
base_free=Vroad|Hstrip|Bay|BayW;
...
for mg=[0.40 0.50 0.60 0.70 0.80]
  ...
  sw=sum(abs(diff(sign(pdir(1:max(np,1))~=0 .* sign(pdir(1:max(np,1))))))>0);
  sw=sum(abs(diff(sign(pdir(pdir~=0))))>0); nrev=sum(pdir(1:np)<0);
  fprintf('margin=%.2f: okF=%d Dr=%.1f okp=%d np=%d switches=%d revPts=%d\n',mg,okF,Dr,okp,np,sw,nrev);
end
disp('SWEEP_DONE');
