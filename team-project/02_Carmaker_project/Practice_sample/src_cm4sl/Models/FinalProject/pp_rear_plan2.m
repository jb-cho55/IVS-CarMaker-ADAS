occ=occ_real; cfg=pp_cfg(); d_r=cfg.d_r; MAXP=cfg.max_path;     % chart-generated map (edited F2)
...
% ===== adaptive reverse-first staging (mirror of planned F6 edit) =====
Dst=cfg.stage_dist; sbx=gx-Dst*cos(gth); sby=gy-Dst*sin(gth);
okF=false; sfx=gx; sfy=gy; Dr=Dst;
while Dr>=1.5
  tfx=gx+Dr*cos(gth); tfy=gy+Dr*sin(gth);
  if ~pp_collision(occ_plan,tfx,tfy,gth,cfg.foot_ahead,cfg.foot_behind,cfg.veh_halfW,cfg.bounds,cfg.n)
    sfx=tfx; sfy=tfy; okF=true; break;
  end
  Dr=Dr-0.5;
end
...
sw=sum(abs(diff(sign(pdir(pdir~=0))))>0);
fprintf('ADAPTIVE: chosen Dr=%.1f okF=%d okB=%d | okp=%d np=%d switches=%d dirv_final=%d\n',Dr,okF,okB,okp,np,sw,dirv);
if np>0
  fprintf('final err=%.2fm ; last6 dir=%s (-1=reverse)\n', hypot(px(np)-gx,py(np)-gy), mat2str(pdir(max(1,np-5):np)'));
  n=200;b=[-5 48 -45.5 6];rx=(b(2)-b(1))/n;ry=(b(4)-b(3))/n;offc=0;
  for i=1:np, cx=max(min(floor((px(i)-b(1))/rx)+1,n),1); cy=max(min(floor((py(i)-b(3))/ry)+1,n),1); offc=offc+(occ(cy,cx)>0); end
  fprintf('path points on OCCUPIED: %d of %d (0=good)\n',offc,np);
end
disp('ADAPT_DONE');
