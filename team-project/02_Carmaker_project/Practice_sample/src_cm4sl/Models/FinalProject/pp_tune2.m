...
function dm=clrall(px,py,pth,np,OB,ids,fa,fbk,hw,oL,oW); dm=inf; for q=1:numel(ids); k=ids(q); tx=OB(k,1);ty=OB(k,2);tth=OB(k,3);cco=cos(tth);ss=sin(tth);Tc=[tx+cco*[oL;oL;0;0]-ss*[oW/2;-oW/2;-oW/2;oW/2],ty+ss*[oL;oL;0;0]+cco*[oW/2;-oW/2;-oW/2;oW/2]];Tp=[];for j=1:4,a=Tc(j,:);bb=Tc(mod(j,4)+1,:);tt=linspace(0,1,16)';Tp=[Tp;a+(bb-a).*tt];end; for i=1:np,co=cos(pth(i));si=sin(pth(i));E=[px(i)+co*[fa;fa;-fbk;-fbk]-si*[hw;-hw;-hw;hw],py(i)+si*[fa;fa;-fbk;-fbk]+co*[hw;-hw;-hw;hw]];Ep=[];for jj=1:4,a=E(jj,:);bb=E(mod(jj,4)+1,:);tt=linspace(0,1,10)';Ep=[Ep;a+(bb-a).*tt];end;D=hypot(Ep(:,1)-Tp(:,1)',Ep(:,2)-Tp(:,2)');dd=min(D(:));if any(inpolygon(Ep(:,1),Ep(:,2),Tc(:,1),Tc(:,2))),dd=0;end;if dd<dm,dm=dd;end;end; end; end
ids=[7 8 9 10 1 13];   % T06 T07 T08 T09 T00 T12
for mg=[0.70 0.80]
  ...
  w=1; M(1:w,:)=1;M(n-w+1:n,:)=1;M(:,1:w)=1;M(:,n-w+1:n)=1; occ=M; occ_plan=pp_clear_goal(occ,fin(1),fin(2),gth,cfg);
  for Dv=[1.5 2.0 2.5]
    sfx=gx+Dv*cos(gth); sfy=gy+Dv*sin(gth); okf=~pp_collision(occ_plan,sfx,sfy,gth,fa,fbk,hw,cfg.bounds,cfg.n);
    px=zeros(MAXP,1);py=px;pth=px;pdir=px;okp=false;np=0;
    ...
    if ~okp, fprintf('mg=%.2f Dr=%.1f: INFEASIBLE\n',mg,Dv); continue; end
    sw=sum(abs(diff(sign(pdir(pdir~=0))))>0); L=sum(hypot(diff(px(1:np)),diff(py(1:np)))); cmin=clrall(px,py,pth,np,OB,ids,fa,fbk,hw,oL,oW);
    fprintf('mg=%.2f Dr=%.1f: np=%d len=%.1f switches=%d MINclr=%.2fm\n',mg,Dv,np,L,sw,cmin);
  end
end
disp('TUNE_DONE');
