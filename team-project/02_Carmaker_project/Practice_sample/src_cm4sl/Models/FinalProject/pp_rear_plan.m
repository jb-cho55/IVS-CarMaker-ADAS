occ=occ_rear; cfg=pp_cfg(); d_r=cfg.d_r; MAXP=cfg.max_path;
ego_x=-0.294; ego_y=-34.512; ego_yaw=deg2rad(-52.9); fin=[9 -11]; gth=pi/2; Dst=2.0;
...
sfx=gx+Dst*cos(gth); sfy=gy+Dst*sin(gth);   % REVERSE-IN staging (north of slot)
okF=~pp_collision(occ_plan,sfx,sfy,gth,cfg.foot_ahead,cfg.foot_behind,cfg.veh_halfW,cfg.bounds,cfg.n);
px=zeros(MAXP,1);py=px;pth=px;pdir=px;
...
fprintf('REAR park (9,-11,+pi/2) Dst=2.0: okF=%d okp=%d np=%d switches=%d\n',okF,okp,np,sw);
if np>0
  fprintf('path X[%.2f,%.2f] Y[%.2f,%.2f]  final err=%.2fm\n',min(px(1:np)),max(px(1:np)),min(py(1:np)),max(py(1:np)),hypot(px(np)-gx,py(np)-gy));
  fprintf('last 8 dir (-1=reverse): %s\n', mat2str(pdir(max(1,np-7):np)'));
  ...
  assignin('base','RR',struct('px',px(1:np),'py',py(1:np),'pdir',pdir(1:np),'np',np,'gx',gx,'gy',gy,'gth',gth,'ex',ex,'ey',ey,'eth',eth,'sfx',sfx,'sfy',sfy,'occ',occ));
end
disp('REAR_DONE');
