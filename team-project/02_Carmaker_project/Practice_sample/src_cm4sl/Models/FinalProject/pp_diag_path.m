...
fprintf('okp=%d np=%d Dr=%.1f\n',okp,np,Dr);
% print path as gear segments
d=pdir(1:np); seg_start=1; fprintf('--- gear segments (dir +1 fwd / -1 rev) ---\n');
for i=2:np, if sign(d(i))~=sign(d(i-1)) || i==np, e2=i-1; if i==np, e2=i; end; L=sum(hypot(diff(px(seg_start:e2)),diff(py(seg_start:e2)))); fprintf(' dir=%+d  (%.1f,%.1f)->(%.1f,%.1f)  len=%.1fm  hdg %.0f->%.0f deg\n', d(seg_start), px(seg_start),py(seg_start),px(e2),py(e2), L, rad2deg(pth(seg_start)), rad2deg(pth(e2))); seg_start=i; end; end
fprintf('path Y range [%.1f, %.1f], X range [%.1f, %.1f]\n', min(py(1:np)),max(py(1:np)),min(px(1:np)),max(px(1:np)));
% min footprint clearance to each row-B/C car
fa=cfg.foot_ahead; fbk=cfg.foot_behind; hw=cfg.veh_halfW; names={'T06','T07','T08','T09','T10','T11','T00','T12'}; idxs=[7 8 9 10 11 12 1 13];
for q=1:numel(idxs), k=idxs(q); tx=OB(k,1);ty=OB(k,2);tth=OB(k,3); cco=cos(tth);ss=sin(tth); Tc=[tx+cco*[oL;oL;0;0]-ss*[oW/2;-oW/2;-oW/2;oW/2], ty+ss*[oL;oL;0;0]+cco*[oW/2;-oW/2;-oW/2;oW/2]]; Tp=[]; for j=1:4,a=Tc(j,:);bb=Tc(mod(j,4)+1,:);tt=linspace(0,1,20)';Tp=[Tp;a+(bb-a).*tt];end; dm=inf;ia=0; for i=1:np, co=cos(pth(i));si=sin(pth(i)); E=[px(i)+co*[fa;fa;-fbk;-fbk]-si*[hw;-hw;-hw;hw], py(i)+si*[fa;fa;-fbk;-fbk]+co*[hw;-hw;-hw;hw]]; Ep=[];for j=1:4,a=E(j,:);bb=E(mod(j,4)+1,:);tt=linspace(0,1,14)';Ep=[Ep;a+(bb-a).*tt];end; D=hypot(Ep(:,1)-Tp(:,1)',Ep(:,2)-Tp(:,2)'); dd=min(D(:)); if any(inpolygon(Ep(:,1),Ep(:,2),Tc(:,1),Tc(:,2))),dd=0;end; if dd<dm,dm=dd;ia=i;end; end; fprintf(' clearance to %s = %.2fm (at idx %d, dir %+d, y=%.1f)\n', names{q}, dm, ia, d(max(ia,1)), py(max(ia,1))); end
disp('DIAG_DONE');
