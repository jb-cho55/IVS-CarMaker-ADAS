...
fa=cfg.foot_ahead; fbk=cfg.foot_behind; hw=cfg.veh_halfW;
clrfn=@(px,py,pth,np,k) deal(0);
function dm=clr2(px,py,pth,np,OB,k,fa,fbk,hw,oL,oW); tx=OB(k,1);ty=OB(k,2);tth=OB(k,3);cco=cos(tth);ss=sin(tth);Tc=[tx+cco*[oL;oL;0;0]-ss*[oW/2;-oW/2;-oW/2;oW/2],ty+ss*[oL;oL;0;0]+cco*[oW/2;-oW/2;-oW/2;oW/2]];Tp=[];for j=1:4,a=Tc(j,:);bb=Tc(mod(j,4)+1,:);tt=linspace(0,1,18)';Tp=[Tp;a+(bb-a).*tt];end;dm=inf;for i=1:np,co=cos(pth(i));si=sin(pth(i));E=[px(i)+co*[fa;fa;-fbk;-fbk]-si*[hw;-hw;-hw;hw],py(i)+si*[fa;fa;-fbk;-fbk]+co*[hw;-hw;-hw;hw]];Ep=[];for j=1:4,a=E(j,:);bb=E(mod(j,4)+1,:);tt=linspace(0,1,12)';Ep=[Ep;a+(bb-a).*tt];end;D=hypot(Ep(:,1)-Tp(:,1)',Ep(:,2)-Tp(:,2)');dd=min(D(:));if any(inpolygon(Ep(:,1),Ep(:,2),Tc(:,1),Tc(:,2))),dd=0;end;if dd<dm,dm=dd;end;end; end
opts={'direct',-1; 'Dr1.5',1.5; 'Dr2',2.0; 'Dr3',3.0};
for o=1:size(opts,1)
  px=zeros(MAXP,1);py=px;pth=px;pdir=px; okp=false; np=0; lab=opts{o,1}; Dv=opts{o,2};
  if Dv<0, [px,py,pth,pdir,np,okp]=pp_hybrid_astar(ex,ey,eth,gx,gy,gth,occ_plan);
  else, sfx=gx+Dv*cos(gth); sfy=gy+Dv*sin(gth); okf=~pp_collision(occ_plan,sfx,sfy,gth,fa,fbk,hw,cfg.bounds,cfg.n); if okf,[px,py,pth,pdir,np,okp]=pp_hybrid_astar(ex,ey,eth,sfx,sfy,gth,occ_plan); if okp,[px,py,pth,pdir,np]=pp_append_straight(px,py,pth,pdir,np,gx,gy,gth,cfg.ds,MAXP,-1);end; end; end
  if ~okp, fprintf('%-7s: INFEASIBLE\n',lab); continue; end
  sw=sum(abs(diff(sign(pdir(pdir~=0))))>0); L=sum(hypot(diff(px(1:np)),diff(py(1:np))));
  c6=clr2(px,py,pth,np,OB,7,fa,fbk,hw,oL,oW); c9=clr2(px,py,pth,np,OB,10,fa,fbk,hw,oL,oW); c7=clr2(px,py,pth,np,OB,8,fa,fbk,hw,oL,oW);
  fprintf('%-7s: np=%d len=%.1fm switches=%d finaldir=%d | clr T06=%.2f T07=%.2f T09=%.2f\n',lab,np,L,sw,pdir(np),c6,c7,c9);
end
disp('CMP_DONE');
