% ===== REPLAN on the ACTUAL edited-chart map (occ_real from base) =====
occ = occ_real;                       % map produced by edited pp_generate_map+pp_add_obstacle
...
fprintf('CHARTMAP: okp=%d np=%d switches=%d tplan=%.1fs\n',okp,np,sw,tplan);
if np>0, fprintf('path X[%.2f,%.2f] Y[%.2f,%.2f]  final err=%.2fm\n',min(px(1:np)),max(px(1:np)),min(py(1:np)),max(py(1:np)),hypot(px(np)-gx,py(np)-gy)); end
% on-surface check
...
fprintf('path points on OCCUPIED cells: %d of %d (0=good)\n', offc, np);
disp('CHARTMAP_DONE');
