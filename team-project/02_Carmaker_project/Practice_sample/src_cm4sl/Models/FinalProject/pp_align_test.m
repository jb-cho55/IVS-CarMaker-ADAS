...
ego_x=-0.294; ego_y=-34.512; ego_yaw=deg2rad(-52.9); fin=[9 -6.8]; gth=-pi/2;   % rear-bumper aligned to T01 (y=-6.8)
...
gf=~pp_collision(occ_plan,gx,gy,gth,cfg.foot_ahead,cfg.foot_behind,cfg.veh_halfW,cfg.bounds,cfg.n);
...
co=cos(gth);si=sin(gth); rbx=px(np)-cos(gth)*0.95; rby=py(np)-sin(gth)*0.95;  % rear bumper = axle - foot_behind*heading
% NOTE foot_behind dir: rear bumper is OPPOSITE heading from axle
rbx=px(np)+(-cos(gth))*0; rby=py(np)+ (0.95)*(-sin(gth))*0;  % placeholder
rb_y = py(np) + cfg.foot_behind*(-sin(gth));   % rear bumper y
nrev=sum(pdir(1:np)<0);
fprintf('ALIGN goal (9,-6.8,-pi/2): gf=%d okF=%d Dr=%.1f okp=%d np=%d switches=%d revPts=%d onOCC=%d err=%.2f\n',gf,okF,Dr,okp,np,sw,nrev,offc,hypot(px(np)-gx,py(np)-gy));
fprintf('ego rear-bumper Y=%.2f  (T01 rear-bumper Y=-6.80)  -> aligned diff=%.2fm\n', rb_y, rb_y-(-6.8));
fprintf('reverse run length (last contiguous reverse) approx: %.1fm\n', nrev*cfg.ds);
assignin('base','AR',struct('px',px(1:np),'py',py(1:np),'pdir',pdir(1:np),'np',np,'gx',gx,'gy',gy,'gth',gth,'ex',ex,'ey',ey,'eth',eth,'sfx',sfx,'sfy',sfy,'occ',occ,'OB',OB,'xc',xc,'yc',yc));
disp('ALIGN_DONE');
