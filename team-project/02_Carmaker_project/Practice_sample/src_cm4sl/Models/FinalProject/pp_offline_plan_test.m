% ===== OFFLINE PARKING REPLAN TEST (corrected lot bottom) =====
n=200; b=[-5 48 -45.5 6]; resx=(b(2)-b(1))/n; resy=(b(4)-b(3))/n;
xc=b(1)+((1:n)-0.5)*resx; yc=b(3)+((1:n)-0.5)*resy; [XC,YC]=meshgrid(xc,yc);
LOTBOT=-38; em=0.20;
M=ones(n,n);
M(XC>=2+em & XC<=48-em & YC>=LOTBOT+em & YC<=-3-em)=0;
WP=[-0.851 3.445;-0.804 2.068;-0.83 0.629;-0.906 -0.866;-1.008 -2.409;-1.113 -3.99;-1.205 -5.6;-1.277 -7.231;-1.328 -8.875;-1.362 -10.527;-1.384 -12.178;-1.393 -13.819;-1.392 -15.446;-1.387 -17.06;-1.383 -18.655;-1.382 -20.198;-1.384 -21.659;-1.387 -23.036;-1.387 -24.337;-1.379 -25.578;-1.356 -26.774;-1.31 -27.938;-1.229 -29.08;-1.1 -30.2;-0.907 -31.294;-0.636 -32.353;-0.275 -33.376;0.187 -34.363;0.76 -35.309;1.441 -36.21;1.819 -36.641];
halfw=max(2.5-em,0.10); dmin=inf(n,n);
for s=1:size(WP,1)-1
  ax=WP(s,1);ay=WP(s,2);bx=WP(s+1,1);by=WP(s+1,2); vx=bx-ax;vy=by-ay;vv=vx*vx+vy*vy;
  if vv<1e-9, d=hypot(XC-ax,YC-ay); else, t=((XC-ax)*vx+(YC-ay)*vy)/vv; t=min(max(t,0),1); d=hypot(XC-(ax+t*vx),YC-(ay+t*vy)); end
  dmin=min(dmin,d);
end
M(dmin<=halfw)=0; w=1; M(1:w,:)=1;M(n-w+1:n,:)=1;M(:,1:w)=1;M(:,n-w+1:n)=1;
H=pi/2;
...
ego_x=-0.294; ego_y=-34.512; ego_yaw=deg2rad(-52.9); fin=[9 -6]; gth=-pi/2;
...
save('D:\HL_IVS_School\Code\motionplanning\26HL_IVS_ADAS\02_Carmaker_project\Practice_sample\src_cm4sl\Models\FinalProject\replan_result.mat','occ','px','py','pdir','np','okp','staged','ex','ey','eth','gx','gy','gth','fin','ego_x','ego_y','ego_yaw','xc','yc','LOTBOT');
disp('DONE_PLAN');
