% feasibility of REAR (reverse-in) park at (9,-11)
n=200; b=[-5 48 -45.5 6]; rx=(b(2)-b(1))/n; ry=(b(4)-b(3))/n;
xc=b(1)+((1:n)-0.5)*rx; yc=b(3)+((1:n)-0.5)*ry; [XC,YC]=meshgrid(xc,yc); em=0.20;
Vroad=(XC>=-2.5+em&XC<=2.5-em&YC>=-36.6&YC<=6); Hstrip=(XC>=0&XC<=47.6-em&YC>=-39.1+em&YC<=-34.1);
Bay=(XC>=5.5+em&XC<=47.6-em&YC>=-34.1&YC<=-4.1-em); BayW=(XC>=4&XC<=5.5&YC>=-34.1&YC<=-30.1);
...
mg=0.40; obs_L=4.47; obs_W=1.97; ah=obs_L+mg; bh=mg; ohw=obs_W/2+mg;
for k=1:size(OB,1)
  x=OB(k,1);y=OB(k,2);th=OB(k,3); co=cos(th);si=sin(th); lx=[ah;ah;-bh;-bh]; ly=[ohw;-ohw;-ohw;ohw];
...
for gth=[pi/2 -pi/2]
  ...
  fprintf('--- gth=%+.4f  goal_footprint_free=%d (axle %.2f,%.2f) ---\n',gth,gf,gx,gy);
  for Dst=[5 4 3 2.5 2]
    sfx=gx+Dst*cos(gth);sfy=gy+Dst*sin(gth); sbx=gx-Dst*cos(gth);sby=gy-Dst*sin(gth);
    ...
    fprintf('   Dst=%.1f  okF(reverse-in,stage N)=%d  okB(forward-in,stage S)=%d\n',Dst,okF,okB);
  end
end
assignin('base','occ_rear',occ); disp('FEAS_DONE');
