...
fprintf('T09 inflation (margin 0.7): (12.5,-20)=%d expect1  (12.0,-20)=%d expect0\n', chk(12.5,-20), chk(12.0,-20));
fprintf('goal (9,-6.8)=%d expect0(free) | entry (-0.29,-34.5)=%d expect0\n', chk(9,-6.8), chk(-0.29,-34.5));
H=-pi/2; aL=3.73; aB=0.95; bw=0.94; gx2=9; gy2=-6.8; co=cos(H);si=sin(H);
xs=gx2+co*[aL;aL;-aB;-aB]-si*[bw;-bw;-bw;bw]; ys=gy2+si*[aL;aL;-aB;-aB]+co*[bw;-bw;-bw;bw];
[XX,YY]=meshgrid(b(1)+((1:n)-0.5)*rx,b(3)+((1:n)-0.5)*ry); ip=inpolygon(XX,YY,xs,ys);
fprintf('goal footprint occupied cells=%d of %d (expect 0)\n', sum(occ(ip)>0), sum(ip(:)));
disp('MAPCHK_DONE');
...
cfg.margin      = 0.70;                   % obstacle footprint safety inflation [m] (raised 0.40->0.70: more clearance from parked cars e.g. T09 during reverse, for tracking error)
