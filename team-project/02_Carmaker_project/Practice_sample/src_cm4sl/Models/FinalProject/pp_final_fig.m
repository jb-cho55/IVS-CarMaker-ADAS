n=200; b=[-5 48 -45.5 6]; rx=(b(2)-b(1))/n; ry=(b(4)-b(3))/n; xc=b(1)+((1:n)-0.5)*rx; yc=b(3)+((1:n)-0.5)*ry; [XC,YC]=meshgrid(xc,yc); em=0.20; mg=0.70;
...
ego_x=-0.294; ego_y=-34.512; ego_yaw=deg2rad(-52.9); fin=[9 -6.8]; gth=-pi/2;
...
Dst=cfg.stage_dist; okF=false; sfx=gx; sfy=gy; Dr=Dst;
...
px=zeros(MAXP,1);py=px;pth=px;pdir=px; [px,py,pth,pdir,np,okp]=pp_hybrid_astar(ex,ey,eth,sfx,sfy,gth,occ_plan); if okp,[px,py,pth,pdir,np]=pp_append_straight(px,py,pth,pdir,np,gx,gy,gth,cfg.ds,MAXP,-1);end
sw=sum(abs(diff(sign(pdir(pdir~=0))))>0); fprintf('mg=0.7 plan: okp=%d np=%d switches=%d revPts=%d\n',okp,np,sw,sum(pdir(1:np)<0));
fig=figure('Color','w','Position',[60 40 900 850]); hold on;
img=cat(3,1-0.75*(occ>0),1-0.7*(occ>0),1-0.55*(occ>0)); image(xc,yc,img); set(gca,'YDir','normal');
for k=1:size(OB,1), x=OB(k,1);y=OB(k,2);th=OB(k,3); co=cos(th);si=sin(th); c=[x+co*[oL;oL;0;0]-si*[oW/2;-oW/2;-oW/2;oW/2], y+si*[oL;oL;0;0]+co*[oW/2;-oW/2;-oW/2;oW/2]]; cl=[0.55 0.6 0.66]; if k==10, cl=[0.95 0.55 0.2]; end; patch(c([1:4 1],1),c([1:4 1],2),cl,'EdgeColor',[0.2 0.22 0.26]); text(x+co*oL/2,y+si*oL/2,sprintf('T%02d',k-1),'FontSize',6.5,'Color','w','HorizontalAlignment','center'); end
fwd=pdir(1:np)>=0; plot(px(1:np),py(1:np),'-','Color',[.5 .5 .5]); scatter(px(find(fwd)),py(find(fwd)),14,'filled','MarkerFaceColor',[0 0.5 0.95]); scatter(px(find(~fwd)),py(find(~fwd)),20,'filled','MarkerFaceColor',[0.95 0.2 0]);
...
de(ex,ey,eth,[1 0.7 0.2]); de(px(np),py(np),gth,[0.2 0.8 0.3]);
yline(-6.8,'m--','후면범퍼 정렬 y=-6.8','LineWidth',1.2);
text(14,-24.5,'T09 (주황)','Color',[0.8 0.4 0],'FontWeight','bold','FontSize',9,'HorizontalAlignment','center');
...
title({'\bf안전여유 0.7m로 재계획: T09 등 주차차량서 \geq1m 떨어져 후진','\rm빨강=후진, 주황=T09'},'FontSize',12);
png='D:\HL_IVS_School\Code\motionplanning\26HL_IVS_ADAS\02_Carmaker_project\Practice_sample\src_cm4sl\Models\FinalProject\parking_safe_reverse.png'; exportgraphics(fig,png,'Resolution',150); fprintf('saved %s\n',png);
disp('FINALFIG_DONE');
