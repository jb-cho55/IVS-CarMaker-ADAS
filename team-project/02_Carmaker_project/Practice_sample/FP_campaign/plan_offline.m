function [px,py,pth,pdir,np,ok] = plan_offline(ego_x,ego_y,ego_yaw,fx,fy,goal_yaw,occ)
cfg=pp_cfg(); d_r=cfg.d_r;
[ex,ey]=pp_axle_from_fr1(ego_x,ego_y,ego_yaw,d_r); eth=ego_yaw; gth=goal_yaw;
[gx,gy]=pp_axle_from_fr1(fx,fy,gth,d_r);
occ2=pp_clear_goal(occ,fx,fy,gth,cfg);
[px,py,pth,pdir,np,ok]=pp_hybrid_astar(ex,ey,eth,gx,gy,gth,occ2);
end
...
switch state
    ...
    case uint8(1)                                       % PLAN (one heavy step)
        occ_plan = pp_clear_goal(occ_map, finish_point(1), finish_point(2), gth, cfg);  % 동적 holdout: 목표슬롯의 차량을 occ에서 제거(목표 도달 가능)
        [px, py, pth, pdir, np, ok] = pp_hybrid_astar(ex, ey, eth, gx, gy, gth, occ_plan);
        if ok
    ...
    case uint8(3)                                       % CORRECT = PRECISION PARKING (정밀주차)
        ...
        if replan
            ...
            if okr
                [rx, ry, rth, rdir, rns] = pp_rs_sample(ex, ey, eth, Tr, sLr, cfg.Rmin, cfg.ds, MAXP);
                if rns >= 2
        ...
        if state == uint8(3)
            [delta, v_des, seg, done] = pp_track(ex, ey, eth, ego_v, PX, PY, PDIR, NP, seg, cfg);
            perr_now = hypot(gx-ex, gy-ey);
            if perr_now < best_perr, best_perr = perr_now; end   % 최근접 추적(running min, 매 스텝)
            if perr_now > best_perr + cfg.creep_margin           % 최선보다 멀어짐(creep/발산) -> 즉시 정지
                state = uint8(4);
            elseif done
                if (perr_now < cfg.align_pos && abs(pp_angdiff(eth,gth)) < cfg.align_yaw) ...
                        || nround >= cfg.max_rounds
                    state = uint8(4);                   % 수렴 또는 라운드한계 -> 정지
                else
                    replan = true;                      % 개선여지 -> 다음 committed 라운드
                end
            end
        end
...
cfg.obs_L       = 4.47;
cfg.obs_W       = 1.97;
cfg.holdout     = 14;                     % pp_obstacles row kept FREE as the parking
...
cfg.ds          = 0.6;                    % [튜닝] motion step(arc)[m]. 작게=충돌검사/경로정밀↑·계산량↑
