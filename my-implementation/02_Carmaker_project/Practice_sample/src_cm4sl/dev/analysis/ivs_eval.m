...
%   Metrics: toll passes, branch visits, entrance reached, T22 & overall min
%   footprint gap (oriented box), overlap count, stopped %, min v at toll.
if nargin<1, N=5; end
...
R=struct('tend',{},'nToll',{},'nBranch',{},'reachEnt',{},'tEnt',{},'t22gap',{},...
         'mingap',{},'minobj',{},'nOverlap',{},'stopPct',{},'vToll',{});
...
for r=1:N
  ...
  % footprint gaps: ALL frames in each object's encounter window (the worst-footprint
  % frame is NOT the min-center frame), using TRUE traffic heading when logged.
  ...
  for k=16:28
    ...
    if gbest<0.5, nOv=nOv+1; end        % contact/near-contact (0.5 m margin for ref-point uncertainty)
  ...
  R(r)=struct('tend',t(end),'nToll',nToll,'nBranch',nBr,'reachEnt',reachEnt,'tEnt',tEnt,...
     't22gap',t22gap,'mingap',mingap,'minobj',minobj,'nOverlap',nOv,'stopPct',100*mean(v<0.5),'vToll',vT);
  fprintf('  run%d: toll=%d br=%d ent=%d(t%.0f) T22gap=%+.2f minGap=%+.2f(T%02d) nOvlp=%d stop=%.0f%% vToll=%.2f\n',...
    r,nToll,nBr,reachEnt,tEnt,t22gap,mingap,minobj,nOv,R(r).stopPct,vT);
end
% aggregate
tolls=[R.nToll]; ents=[R.reachEnt]; ovl=[R.nOverlap]; t22=[R.t22gap]; mg=[R.mingap]; sp=[R.stopPct];
fprintf('--- AGG [%s] ---\n', tag);
...
fprintf('  runs with any overlap   : %d/%d (%.0f%%)\n', sum(ovl>0), N, 100*mean(ovl>0));
fprintf('  T22 overlap (gap<0)     : %d/%d (%.0f%%)\n', sum(t22<0), N, 100*mean(t22<0));
