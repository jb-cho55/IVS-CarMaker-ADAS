...
if ua<0.5
  ...
  % --- APPROACH curvature slowdown: modemgr raises vset on the entry STRAIGHT so
  %     ego outruns the tail-gating ego-coupled T22 (which rear-ends on hard
  %     decel). Here we keep that speed safe through the bend (a_lat<=3) so the
  %     raised vset does not derail on the right-hand turn into the entrance. ---
  kmax=0;
  for j=2:min(Np-1,26)
    a1x=Path(2*j-1)-Path(2*j-3); a1y=Path(2*j)-Path(2*j-2); a2x=Path(2*j+1)-Path(2*j-1); a2y=Path(2*j+2)-Path(2*j);
    l1=sqrt(a1x*a1x+a1y*a1y); l2=sqrt(a2x*a2x+a2y*a2y);
